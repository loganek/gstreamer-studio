...
"$enum_name EnumUtils<$enum_name>::string_to_enum(const std::string& enum_value)\n";
	$ret .= "{\n";
	$ret .= "\tstd::string val = boost::to_upper_copy(enum_value);\n\n";
...
$header = basename($ARGV[1]);
print $output_file_cpp "#include \"$header\"\n";
print $output_file_cpp "#include <boost/algorithm/string.hpp>\n";
print $output_file_cpp "#include <stdexcept>\n\n";

if ( $ARGV[0] =~ m/src\/utils\/\.\.\/(.*)$/ ) {
	$include_str = $1;
}
else {
	$include_str = $ARGV[0];
...
print $output_file_cpp "using namespace GstreamerStudio::LeParXer;\n\n";
